...
import (
	...
	_ "github.com/rh-ecosystem-edge/eco-gotests/tests/assisted/ztp/ibio-tls/tests"
	"github.com/rh-ecosystem-edge/eco-gotests/tests/internal/reporter"
)

var _, currentFile, _, _ = runtime.Caller(0)

func TestIBIOTLS(t *testing.T) {
	_, reporterConfig := GinkgoConfiguration()
	reporterConfig.JUnitReport = GeneralConfig.GetJunitReportPath(currentFile)

	RegisterFailHandler(Fail)
	RunSpecs(t, "IBIO TLS Profile Suite", Label(tsparams.Labels...), reporterConfig)
}
...
var _ = BeforeSuite(func() {
	...
	if HubAPIClient == nil {
		Skip("Cannot run IBIO TLS Profile suite when hub has nil api client")
	}
})
